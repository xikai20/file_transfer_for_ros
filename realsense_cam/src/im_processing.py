...
def find_box_points(req):
    ...
    pcl = pcl_res.pointcloud
    xyz_array = ros_numpy.point_cloud2.pointcloud2_to_xyz_array(pcl, False)
    points = xyz_array[min_y+150:max_y-150,min_x+150:max_x-150]
    print(points.shape)
    ...
    box_pcl.fields = pcl.fields
    box_pcl.is_bigendian = pcl.is_bigendian
    box_pcl.point_step = pcl.point_step
    ...
    for i in range(points.shape[1]/19):
        for j in range(points.shape[0]/2):
            # rospy.loginfo("i: %i j: %i" % (i,j))
            test_point = geometry_msgs.msg.PointStamped()
            test_point.point.x = points[j*2][i*19][0]
            test_point.point.y = points[j*2][i*19][1]

            # Check the next few neighbors for the highest height to try to avoid collisions.
            try:
                neighbor_zs = points[(j-1)*2:((j+1)*2)][(i-1)*2:(i+1)*2][2]
    ...
    return FindBoxPointsResponse(True,results)
