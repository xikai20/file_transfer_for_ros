...
class RealSenseCam
{
	...
	public:
		RealSenseCam() : nh("realsense_services"), pnh("~"), frame_id("cam"), seq(0)
		{
			...
			std::string pointcloud_topic_name = this->pnh.param<std::string>("realsense_pointcloud_topic", "/D415/depth_registered/points"); //Return value from parameter "realsense_pointcloud_topic" if available
