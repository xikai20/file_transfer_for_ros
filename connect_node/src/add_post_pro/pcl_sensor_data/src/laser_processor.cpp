...
class LaserProcessor
{
    ...
    public:
        LaserProcessor() : nh("laser_processor"), pnh("~"), frame_id("scancontrol2900-50_laser"), seq(0)
        {
            ROS_INFO("Starting laser processing services");
            std::string start_srv_name = this->pnh.param<std::string>("start_service_name", "start_recording");		//Get string name from parameters or use default
            ...
            std::string stop_srv_name = this->pnh.param<std::string>("stop_service_name", "stop_recording");		//Get string name from parameters or use default
			this->stop_srv  = this->nh.advertiseService(stop_srv_name, &LaserProcessor::stop_recording, this);
            ROS_INFO("Initialized stop service");

            std::string restart_srv_name = this->pnh.param<std::string>("restart_service_name", "restart_recording");		//Get string name from parameters or use default
            ...
            std::string process_srv_name = this->pnh.param<std::string>("process_service_name", "process_pointcloud");		//Get string name from parameters or use default
        ...
        void pcl_callback(const sensor_msgs::PointCloud2ConstPtr &scan_in)
        {
            ...
            try{
                this->listener.waitForTransform("/world", "scancontrol2900-50_laser", scan_in->header.stamp, ros::Duration(0.1));
                this->listener.lookupTransform("/world", "scancontrol2900-50_laser",  scan_in->header.stamp, stamped_transform);
